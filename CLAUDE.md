...
- ⚠️ **Because the workers live in the API process, every stray backend process is a competing consumer — and an orphaned one runs stale code.** This caused a real, baffling bug on 2026-08-31: auto-resolve appeared to work intermittently (ticket resolved, reply written) but no email was sent and *nothing was logged*, because two orphaned dev servers from six days earlier were still polling `ticket-auto-resolve`. Their code predated outbound email, so whenever one won a job it wrote the reply and never called `enqueueEmailSend`. Whichever process grabs a job decides the behaviour, so a zombie makes bugs look random.
  - **Symptom to recognise**: a resolved ticket whose outbound `Message` has a **null `messageId`**. Current code always mints one (`newOutboundMessageId`), so a null on an AI reply means an old process wrote it.
  - **Diagnose**: `select date_trunc('minute', backend_start), count(*) from pg_stat_activity where datname='helpdesk' group by 1 order by 1;` — a connection far older than your current server is the tell. Then `Get-CimInstance Win32_Process -Filter "Name='bun.exe'" | Select ProcessId, CreationDate, CommandLine` and look for extra `run src/index.ts` processes. Note a `bun run dev` shows up as a parent plus a `--watch run src/index.ts` child, so one server is two processes.
  - **Prevent**: on Windows, `taskkill /F` on the port-3001 listener kills only the process holding the port — a `bun run start`/`--watch` sibling can survive and keep consuming jobs. After killing a dev server, confirm no `src/index.ts` process is left before starting a new one.
...
- ⚠️ **A `resolve` that falls below the threshold throws the drafted reply away.** The model has already written the full reply by then, but the escalation path stores only `aiConfidence`/`aiDecision` — the text is not persisted anywhere. This is the first thing to check when someone reports "the auto-responder didn't answer": read `aiDecision`, which spells out `Confidence X is below the Y threshold` along with the section the model believed answered the ticket. It looks like a failure and is really a tuning outcome. (If the drafted reply ever needs to survive for an agent to edit, that's a schema change — nothing captures it today.)
...
- **Knowledge base**: `backend/knowledge-base.md`, loaded and cached by `backend/src/tickets/knowledge-base.ts` (resolved relative to the source, not the cwd; override with `KNOWLEDGE_BASE_PATH`). It ships in the system prompt behind a `cache_control` breakpoint since it is byte-identical on every job. Its §10 escalation rules are policy, not code — edit the markdown, not the prompt. **It is cached per process, and `bun --watch` doesn't watch `.md` — restart the backend after editing it**, or the worker keeps answering from the old copy.
- ⚠️ **The knowledge base's formatting measurably changes the model's confidence.** It arrived as pandoc output with its bullet lists flattened into run-on prose (`Possible reasons: - You are logged in with... - The payment is still processing.`). Repairing the lists took the *identical* question ("I purchased a course but cannot see it", KB §2) from **0.75 to 0.88** confidence — the difference between being escalated and being answered. Two rules follow: keep the markdown well-formed, and give each answer a concrete "What to do" sequence, because the model is scoring how completely it can answer from what it is given. Anything copied in from another tool should be checked for flattened lists and `--`/`5--10` artifacts, which otherwise reach the customer verbatim in a reply.
- **Env**: `AUTO_RESOLVE_ENABLED`, `AUTO_RESOLVE_MODEL`, `AUTO_RESOLVE_CONFIDENCE_THRESHOLD`, `POSTMARK_FROM_EMAIL` (the reply's From and attribution — falls back to `SUPPORT_EMAIL`), `SUPPORT_NAME`, `AI_AGENT_EMAIL` — see `backend/.env.example`.
- **The threshold is a live tuning dial, and 0.8 was too tight for this knowledge base.** The code default is 0.8; this deployment currently sets `AUTO_RESOLVE_CONFIDENCE_THRESHOLD=0.7`, added while diagnosing a report that no auto-response was sent. Useful calibration from that session: junk or genuinely account-specific tickets escalate at **0.95–0.98** (confident *escalates*, which is the system working), while questions the knowledge base really answers scored **0.70–0.75** — only just under the gate. Repairing the knowledge base's markdown lifted the same questions to **0.85–0.88**, i.e. above even the original 0.8, so the 0.7 setting is now belt-and-braces rather than load-bearing and can reasonably go back to 0.8. Fix the knowledge base before loosening the gate.
...
Email arrives over a webhook and becomes a ticket. Sending is covered in "Outbound email (Postmark)" below; both directions work.
...
- **We mint the Message-Id, Postmark doesn't give us one.** The send API returns its own delivery UUID, not an RFC `Message-Id`, and the id has to exist before the send so it can be stored on the row. `newOutboundMessageId()` in `backend/src/tickets/outbound-message.ts` generates it, rooted in the sending address' domain (`supportIdentity()`, so `POSTMARK_FROM_EMAIL` before `SUPPORT_EMAIL`), stored bare so it compares equal to the bracket-stripped ids the inbound side normalizes to.
...
- ~~While a Postmark account is pending approval it only accepts recipients on the From address' own domain.~~ **Resolved — the account was approved on 2026-08-31** and verified sending to an external (Gmail) recipient. Kept here because the handling is still the general case: a restriction like that arrives as an `ApiInputError`, which is recorded on the message and *completes* the job rather than burning retries. Don't add addressing workarounds for that class of rejection.
...
- `bun run test` — the backend unit suite (`bun test`); `bun run test:watch` to iterate
...
Backend unit tests run on **`bun test`** (not Vitest), colocated as `backend/src/**/*.test.ts`. Run `bun run test` from `backend/`. This is where the ingest/threading/queue/AI logic is covered, since none of it is reachable from a component test and it is far too detailed for e2e.

- ⚠️ **`mock.module` is process-global and hoisted, so two specs cannot hold different stubs of the same module** — whichever registers first binds it for every file in the run. The consequence is counter-intuitive: **a feature gets one spec file, not one per module.** `auto-resolve.test.ts` and `outbound-email.test.ts` each cover several modules for exactly this reason, and both carry a header comment saying so. Splitting either one will break the other's assertions, and the failure looks like a bug in the code under test rather than in the mocks. This bit during the outbound work: a separate `email-queue.test.ts` that stubbed `send-email.ts` silently replaced the real module that `send-email.test.ts` was testing, and 18 unrelated tests failed only when the whole suite ran.
- **Mock at the leaf boundaries only** — the Anthropic SDK, the Postmark SDK, `../prisma.ts`, `../queue.ts`. Everything first-party runs for real, and outcomes are steered by what the stubbed SDK returns. That is what makes these specs worth having.
- ⚠️ **bun loads the real `.env` into the test process**, so anything set there silently overrides a fixture. Any env var a spec asserts on must be explicitly set *or deleted* in `beforeEach` — `POSTMARK_FROM_EMAIL` and `POSTMARK_INBOUND_ADDRESS` are both real-world values that broke assertions this way. Snapshot and restore `process.env` in `afterEach`.
...
- **Phase 4 — Email (Postmark)**: inbound and outbound both work — see "Inbound email (Postmark)" and "Outbound email (Postmark)" above. Mail arrives at `POST /api/webhooks/postmark/inbound`, and agent and AI replies go out through the `email-send` queue with threading headers, so a student's reply lands back on the same ticket. Local dev goes through a tunnel (the README has the recipe; the Postmark server's inbound hook currently points at an ngrok URL with the secret in the query string). Still open in the phase: attachments (dropped on the way in, never sent on the way out) and spam filtering — Postmark's inbound payload carries `X-Spam-Status` in `Headers[]` if you want to act on it.
